...
TiXmlNode* TiXmlNode::LinkEndChild( TiXmlNode* node )
{
	...
	if ( lastChild )
		...
		curChild = firstChild;      //2009-02-09 wanglei
	}
...
TiXmlElement* TiXmlNode::FirstChildElement( const char * value ) const
{
	...
	return CurChildElement(value);
}
/////////////////////////////////////////////////////////////////
//2009-02-09 wanglei 添加功能
//查找元素的起始位置为 从上一次找到某个元素的位置开始继续查找
TiXmlNode* TiXmlNode::CurChild( const char * value ) const
{
	TiXmlNode* node;
	for ( node = curChild; node; node = node->next )
	{
		if ( node->SValue() == TIXML_STRING( value )){
			curChild = node;
			return node;		
	...
	if (!node && curChild != firstChild){//如果没有找到则从第一个位置开始搜索到curChild
		for (node = firstChild; node != curChild; node = node->next){
			if ( node->SValue() == TIXML_STRING( value )){
				curChild = node;
				return node;
			}
...
TiXmlElement* TiXmlNode::CurChildElement( const char * value ) const
{
	TiXmlNode* node;

	for (	node = CurChild( value );
	node;
	node = node->NextSibling( value ) )
	{
		if ( node->ToElement() )
...
int TiXmlNode::CalcChildrenElementNum(const char* value) const
{
	int cnt = 0;
	TiXmlNode *cur = FirstChildElement(value);
	while (cur) {
		cnt ++;
		cur = cur->NextSiblingElement(value);
	}
	return cnt;
}
...
{
	firstChild = lastChild = curChild = 0;
	value = _value;
}
